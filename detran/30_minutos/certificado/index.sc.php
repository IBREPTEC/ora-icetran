...
$sql = 'SELECT
		...
		(SELECT dl.retorno FROM detran_logs dl where dl.idmatricula=m.idmatricula ORDER BY dl.data_cad DESC LIMIT 1) AS retorno
    FROM
        matriculas m
    ...
    WHERE        
		...
		AND m.data_cad >= "2022-10-01 00:00:00"		
		ORDER BY data_ultimo_historico ASC
	LIMIT 300';

$query = $matriculaObj->executaSql($sql);
while ($linha = mysql_fetch_assoc($query)) {
	//if(strpos($linha['retorno'], 'Verifique com Detran/SC') === false){
