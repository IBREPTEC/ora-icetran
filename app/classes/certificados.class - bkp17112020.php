...
class Certificados extends Core
{
    ...
    public function gerarCertificado($idmatricula, Matriculas $matriculas, $idFolhaRegistro = NULL)
    {
        ...
        if ($information['oferta_curso']['idfolha'] && $matriculas->verificaMatriculaAprovadaNotas($information['oferta_curso']['porcentagem_minima_disciplinas'])) {

            /* Verifica se o curso é o de Reciclagem de Condutores e Infratores e se o estado do CFC é o de maranhão. Essa verificação ocorre para não mudar a situação quando clicar para gerar o diploma, pois é a cron do detran de
            MA que deve alterar para concluido. */
            ...
            if (! $alunoJaPossuiFolha) {
                ...
                if (! empty($folhaRegistro['limite_matriculas']) && $qntMatriculas >= $folhaRegistro['limite_matriculas']) {
                    ...
                    if (! empty($clonar['sucesso'])) {
                        $this->atualizaFolhaRegistroOfertaCurso(
                            $information['oferta_curso']['idoferta_curso'],
                            $clonar['id']
                        );
        ...
        $certificado = $this->set('id', $certificadoCurso['idcertificado'])->retornar();
        $paginas = $this->listarPaginas($certificadoCurso['idcertificado']);
        ...
        $folhaRegistro = new Folhas_Registros_Diplomas();
        $folhas = $folhaRegistro->getRowByIdFolha($idFolhaRegistro);
        $folhaDiplomaMatricula = $folhaRegistro->retornarMatriculasDaFolha($idFolhaRegistro, $idmatricula);
        ...
        $html = '<table width="100%" border="0" cellpadding="2" cellspacing="0" style="border: 1px solid #000; font-size:12px;" class="tableBoletim">
					<tr>
        ...
        $matriculaCurriculo = $matriculas->RetornarCurriculo();//Retorna os dados do currículo da matrícula
        $cargaHorariaTotal = 0;
		foreach ($boletim['aluno']->disciplinas as $disciplina) {
        ...
        $data_inicio_certificado = $dataInicioCurso['data_cad'];
        if(!empty($information['matricula']['data_inicio_certificado'])){
            $data_inicio_certificado = $information['matricula']['data_inicio_certificado'];
        }
        ...
        $fixed = array(
            ...
            '[[extra][data_inicio_curso]]' => ($data_inicio_certificado) ? FiltrosHelper::converterData($data_inicio_certificado, "d/m/Y") : null,
			...
			'[[extra][numero_registro]]' => ($folhaDiplomaMatricula['numero_registro']) ? $folhaDiplomaMatricula['numero_registro'] : null,
			...
			'[[extra][numero_folha]]' => ($folhas['numero_folha']) ? $folhas['numero_folha'] : null,
        ...
        */
        ...
        $subject['[[matricula][codigo_diploma]]'] = $information['matricula']['codigo_diploma'];
        $subject['[[matricula][renach]]'] = $information['pessoa']['ato_punitivo'];
    ...
    $subject['[[sindicato][fone]]'] = $subject['[[sindicato][telefone]]'];
        ...
        $patterns = array(
            '#\[\[midia\]\[(\d+)\]\]#i' => 'SELECT * FROM certificados_midias WHERE idcertificado_midia = %d AND idcertificado = '.$this->id
        );
        ...
        foreach ($paginas as $pagina) {
            $subject['[[extra][ordem_pagina_certificado]]'] =  str_pad($folhaDiplomaMatricula['numero_ordem'], 5, "0", STR_PAD_LEFT);
            ...
            if (file_exists($_tmp_file_location)) {
                ...
                foreach ($patterns as $pattern => $query) {
                    ...
                    foreach ($midiasCollection as $id => $tagToReplace) {
                        ...
                        if ($_query){
                            $content = str_ireplace($tagToReplace, 'http://'.$_SERVER['SERVER_NAME'].'/storage/certificados_midias/'.$_query->idcertificado.'/'.$_query->arquivo_servidor, $content);
                        } else{
                            $content = str_ireplace($tagToReplace, 'http://'.$_SERVER['SERVER_NAME'].'/assets/img/__semimagem_api.jpg', $content);
                        }
                    }
