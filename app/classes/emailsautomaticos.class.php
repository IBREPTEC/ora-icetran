...
class Emails_Automaticos extends Core
{
	...
	function sendWhatsApp($nomePessoa, $celularPessoa)
    {
        $params = json_encode([
            "name" => "$nomePessoa",
            "urns" => ["whatsapp: $celularPessoa"]
        ]);

        $headers = [
            "Authorization: Token f466c0a6b2c1a1c4439489ef6df8130830b5940a",
            "Content-Type: application/json"
        ];

        $curl = curl_init();

        curl_setopt($curl, CURLOPT_URL, "https://flows.weni.ai/api/v2/contacts.json");
        ...
        if ($httpCode != 400) {
            $params = json_encode([
                "flow" => "7d3b4ce5-2923-48fd-8cac-6ad327692fa3",
                "urns" => ["whatsapp: $celularPessoa"]
            ]);

            $curl = curl_init();

            curl_setopt($curl, CURLOPT_URL, "https://flows.weni.ai/api/v2/flow_starts.json");
            curl_setopt($curl, CURLOPT_POST, 1);
            curl_setopt($curl, CURLOPT_POSTFIELDS, $params);
            curl_setopt($curl, CURLOPT_HTTPHEADER, $headers);
	...
	{ 
		...
		if ($message) {
			$coreObj->enviarEmail(utf8_decode($nomeDe), $emailDe, $assunto, $message, $nomePara, $emailPara, 'layout_branco');
		}
	}
