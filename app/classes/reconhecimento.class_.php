...
class Reconhecimento extends Core {
    ...
    public function registrarImagemPrincipal()
    {
        ...
        $probab = 0.85;//$this->getSimilaridade($_POST['idmatricula'],$_FILES);
    ...
    public function compararFotos($useDV=false)
    {
        ...
        $urlImagemMatricula = $GLOBALS['config']['urlSistema'] . '/api/get/imagens/matriculas_reconhecimentos/x/x/' . $imagemMatricula['foto'];
        $urlImagemComparacao = $GLOBALS['config']['urlSistema'] . '/api/get/imagens/matriculas_comparacoes_fotos/x/x/' . $nome_servidor;
        
        //REQUISITA O AZURE PARA NO METODO FACE-DETECT PARA RETORNAR O FACEID E ATRIBUTOS.
        $parameters = array('returnFaceId' => 'true', 'returnFaceLandmarks' => 'false', 'returnFaceAttributes' => 'age,gender');
        $request = new Http_Request2('https://brazilsouth.api.cognitive.microsoft.com/face/v1.0/detect');
        $body = array("url" => $urlImagemMatricula);
        $resultado = json_decode($this->requestDetect($request, $parameters, $body), true);

        //REQUISITA O AZURE PARA NO METODO FACE-DETECT PARA RETORNAR O FACEID E ATRIBUTOS.
        $parameters = array('returnFaceId' => 'true', 'returnFaceLandmarks' => 'false', 'returnFaceAttributes' => 'age,gender');
        $request = new Http_Request2('https://brazilsouth.api.cognitive.microsoft.com/face/v1.0/detect');
        $body = array("url" => $urlImagemComparacao);
        $resultado2 = json_decode($this->requestDetect($request, $parameters, $body), true);
        ...
        if($resultado[0]['faceId'] && $resultado2[0]['faceId']){
            ...
            if(in_array($_POST['idmatricula'], $GLOBALS['config']['reconhecimento']['matriculas_liberadas']) || $resultado3['confidence'] >= $GLOBALS['config']['reconhecimento']['range_minimo']){
                $resultadoComparacao = 'S';
            }
