...
class Core
{
    ...
    {
        return mysql_query("commit") or die(incluirLib("erro", $this->config, array("sql" => "commit", "session" => $_SESSION, "get" => $_GET, "post" => $_POST, "mysql_error" => mysql_error())));
    }

    function executaSql($sql)
    {
        $retorno = mysql_query($sql) or die(incluirLib("erro", $this->config, array("sql" => $sql, "session" => $_SESSION, "get" => $_GET, "post" => $_POST, "mysql_error" => mysql_error())));
        return $retorno;
    }
        ...
        if (is_array($this->config["formulario"])) {
            foreach ($this->config["formulario"] as $fieldsetid => $fieldset) {
                foreach ($fieldset["campos"] as $campoid => $campo) {
                    ...
                    //$extensao =  strtolower(end(explode(".", $this->post[$campo["nome"]]["name"])));
    ...
    function SalvarDados()
    {
        ...
        if ($erros) {
            ...
            //VERIFICANDO SE FORMUL�?RIO TEM ARRAY
            if ($formularioArray) {
                    ...
                    foreach ($arrayCampoSecundario as $nomeTabela => $arrayCampos) {
                        ...
                        if (count($arrayCampos) > 0) {
                            foreach ($arrayCampos as $ind => $campos) {
                                //VERIFICANDO SE REGISTRO J�?�? EXISTE NO BANCO
                                ...
                                //$query_existe = mysql_query($this->sqlAux[0]) or die(incluirLib("erro",$this->config,$this->sql,array("sql" => $this->sql, "mysql_error" => mysql_error())));
                                ...
                                if ($existe == 0) {
                                    ...
                                    //mysql_query($this->sqlAux[1]) or die(incluirLib("erro",$this->config,$this->sql,array("sql" => $this->sql, "mysql_error" => mysql_error())));
                                    //$this->monitora_oque = 1;
                                    //$this->monitora_qual = mysql_insert_id();
                                    //$this->Monitora();
                                }
                        ...
                        //mysql_query($this->sqlAux[2]) or die(incluirLib("erro",$this->config,$this->sql,array("sql" => $this->sql, "mysql_error" => mysql_error())));
                    }
                }
            }
    ...
    function ValidarArquivo($file)
    {
        ...
        //echo  "Entrou Function<br>";
        //echo $file['size']."<br>";
        //echo $exetensao[0]."<br>";
        //echo $this->config['tamanho_upload_padrao'];
        //exit;
...
*/
    ...
    // Colocar para saber se o e-mail é restrito ou nao
    function enviarEmail($nomeDe, $emailDe, $assunto, $mensagem, $nomePara, $emailPara, $layout = "layout", $charset = 'iso-8859-1') {
        ...
        if ($this->reenviar) {
            ...
            $nomeDe =  utf8_encode($email['de_nome']);
            $emailDe = $email['de_email'];
            $assunto =  utf8_encode($email['assunto']);
            $headers = $email['cabecalho'];
            $layout = $email['layout'];
            $nomePara = utf8_encode($email['para_nome']);
            if (! $this->email_reenvio) {
                $this->email_reenvio = $email['para_email'];
            }
            $emailPara = $this->email_reenvio;

            //$layoutHTML = utf8_decode($email['mensagem']);
       ...
       //  exit
        ...
        if (! $this->naoSalvarLogEmail) {
            ...
            $sql = "insert into
                    ...
                    set
                        data_cad = now(),
                        de_nome = '".mysql_real_escape_string(utf8_encode($nomeDe))."',
                        de_email = '".$emailDe."',
                        para_nome = '".mysql_real_escape_string(utf8_encode($nomePara))."',
                        para_email = '".$emailPara."',
                        assunto = '".mysql_real_escape_string(utf8_encode($assunto))."',
                        layout = '".$layout."',
                        mensagem = '".mysql_real_escape_string((utf8_encode($mensagem)))."',
                        cabecalho = '".$headers."'";
       ...
       // echo $layoutHTML;
      //  echo "nome de: " . $nomeDe . "<br>"; 
     //   echo "nome para: " . $nomePara . "<br>"; 
      //  echo "assunto: " . $assunto . "<br>"; 
      //     echo "email: " . $emailPara . "<br>"; 
