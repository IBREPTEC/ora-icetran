...
class Escolas extends Core
{
    public function listarTodas($retornarEstadosCidades = false)
    {
        ...
        if (!empty($retornarEstadosCidades)) {
            foreach ($retorno as $ind => $var) {
                ...
                $this->campos = 'eec.idestado, eec.idcidade, es.nome AS estado, es.sigla AS uf, c.nome AS cidade';
    ...
    public function retornar($retornarEstadosCidades = false)
    {
        $this->sql = 'SELECT
            ...
            FROM
                ...
                LEFT JOIN escolas_estados_cidades eec ON (eec.idescola = e.idescola AND eec.ativo = "S")
        ...
        if ($this->modulo != 'web') {
            $this->sql .= ' AND e.idescola = ' . $this->id;
        }

        if ($this->slug) {
            $this->sql .= ' AND e.slug = "' . $this->slug . '"';
        }
        ...
        if ($this->ativoPainel) {
            $this->sql .= ' AND e.ativo_painel = "' . $this->ativoPainel . '"';
        }

        $this->sql .= ' GROUP BY e.idescola';
    ...
    {
        $sqlCfc = 'SELECT  e.idplano,pc.valor_minimo,pc.idplanos_cfc,pc.idcfc,pc.vencimento FROM escolas e inner join planos_cfc pc on(e.idescola=pc.idcfc) 
        ...
        if ($this->post["idcontrato"]) {
            ...
            $this->sql = "SELECT
                            ...
                            FROM
                                ...
                                LEFT OUTER JOIN estados ge ON (ge.idestado = e.gerente_idestado)
                                LEFT OUTER JOIN cidades gc ON (gc.idcidade = e.gerente_idcidade)

                                LEFT OUTER JOIN estados re ON (re.idestado = e.responsavel_legal_idestado)
                                LEFT OUTER JOIN cidades rc ON (rc.idcidade = e.responsavel_legal_idcidade)
                            WHERE
                                idescola = " . $this->id;
