...
class Fechamentos_Caixa extends Core
{
    ...
    function fecharCaixa()
    {
        ...
        $sql_atualizar_fechamento = " UPDATE fechamentos_caixa SET credito_valor = '".$total_receber['total_valor']."',
    ...
    public function retornarContaFechamento($idfechamento)
    {
        $this->sql = "SELECT
                ...
                COALESCE(SUM(cvv.valor_por_matricula), SUM(svv.valor_por_matricula), e.valor_por_matricula)  AS escola_valor_por_matricula,
            ...
            FROM
                contas c
                INNER JOIN escolas e ON (c.idescola = e.idescola)
                LEFT JOIN cfcs_valores_cursos cvv ON (cvv.idcfc = e.idescola AND cvv.ativo = 'S')
                INNER JOIN cidades cc ON (cc.idcidade = e.idcidade)
