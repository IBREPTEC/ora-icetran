...
class Conteudos extends Ava
{
	...
	function cadastrarConteudoImagem($nomeServidor){




        $sql ="insert into
                        avas_conteudos_imagem
                      set titulo='".$this->post['titulo']."',
                      idconteudo='".$this->post['idconteudo']."',
                      link = '".$this->post['url']."',
                      idava='".$this->post['idava']."',
                      ordem='".$this->post['ordem']."',
                      imagem_exibicao_nome = '".$this->post['arquivo']['name']."',
                      imagem_exibicao_servidor = '".$nomeServidor."',
                      imagem_exibicao_tipo ='".$this->post['arquivo']['type']."',
                      imagem_exibicao_tamanho ='".$this->post['arquivo']['size']."'
                     ";
        $executa =  $this->executaSql($sql);

        if($executa){

            $this->retorno["sucesso"] = true;
            $this->retorno["mensagem"] = 'imagem_salva';
    ...
    function removeImagemConteudo($id){
        $sql = 'delete from avas_conteudos_imagem where idimagem='.$id;
        $executa = $this->executaSql($sql);
    ...
    function retornarDadosImagem($idimagem){
        $sql ="select * from avas_conteudos_imagem where idimagem='".$idimagem."'";
        return $this->retornarLinha($sql);
    }
    function editarImagensConteudo($nomeServidor){
        if($nomeServidor == '') {
            $this->sql = "update
                        avas_conteudos_imagem
                      set
        ...
        $executa = $this->executaSql($this->sql, false);
        if($executa){

            $this->retorno["sucesso"] = true;
            $this->retorno["mensagem"] = 'imagem_modificada';
        } else {
            $this->retorno["erro"] = true;
            $this->retorno["erros"][] = $this->sql;
            $this->retorno["erros"][] = $this->retornarErrorQuery();
    ...
    function retornarImagensConteudo($idava,$idconteudo){
        $this->sql ="select * from avas_conteudos_imagem a where idava='".$idava."' and idconteudo='".$idconteudo."'";
        $this->ordem_campo= 'a.ordem';
        $this->ordem = 'asc';
        $this->groupby = 'a.idimagem';
