						...
						foreach ($arrayParticipantes["professores"] as $ind => $var) {
							...
							if ($this->idpessoa) {
								...
								$message  = "Olá <strong>".$nomePara."</strong>,
												<br /><br />
												O Aluno(a) ".$aluno["nome"].", mandou a seguinte d&uacute;vida:
												<br /><br />";
								...
								$message .= '<br/><br/>
											<a href="'.$GLOBALS['config']["urlSistema"].'/professor/academico/avas/'.$this->idava.'/mensagem_instantanea/'.$idmensagem_instantanea.'">
												Clique aqui para abrir o tira dúvida.
											<a>';
								$message = utf8_decode($message);
								//$message .= utf8_decode($this->post['mensagem']);
								//$message = html_entity_decode($message);

								$this->EnviarEmail($nomeDe, $emailDe, $assunto, $message, utf8_decode($nomePara), $emailPara);
							}
