...
class Matriculas extends Core {
    ...
    public function Retornar() {
	    ...
	    public function retornarConteudoCurso(){
        $avas = $this->retornaIdAvas();
        $retorno = [];
        foreach ($avas as $ava){
            $this->sql = 'SELECT
                        ara.idava,
                        arao.*
                        FROM avas_rotas_aprendizagem ara
        INNER JOIN avas_rotas_aprendizagem_objetos arao ON (ara.idrota_aprendizagem = arao.idrota_aprendizagem and arao.ativo = "S")
        INNER JOIN matriculas_rotas_aprendizagem_objetos mrao on (mrao.idobjeto=arao.idobjeto)
        WHERE
        ara.idava = '.$ava.' AND
       ...
       ';
            ...
            foreach ($objetosRota as $ind => $objetoRota) {
                $retorno[$ind]['objeto'] = $this->retornarObjeto($objetoRota);
            }
	...
	public function retornarConteudoEspecifico($idava,$idobjeto){
        $retorno = [];
            $this->sql = 'SELECT
                        ara.idava,
                        arao.*
       ...
       ';
            ...
            $this->limite = 1;
            $this->groupby = 'arao.idobjeto';
            $objetosRota = $this->retornarLinhasArray($this->sql);

        $idconteudo = $objetosRota[0]['idconteudo'];
        $sql_conteudo = 'select nome from avas_conteudos where idconteudo='.$idconteudo;

        return $this->retornarLinha($sql_conteudo);
    ...
    public function retornarObjeto($objetoRota)
    {
        switch ($objetoRota["tipo"]) {
            ...
            case 'simulado':
                ...
                break;
			//case 'reconhecimento':
                //$objeto = "Falta Reconhecimento Facil";
                //break;
            case 'enquete':
    ...
    public function retornarAvaMatricula($idava)
    {
	    ...
	    public function totalContabilizado($idava, $tipo, $id) {
        ...
        $retorno = $this->retornarLinhasArray($this->sql);

        return  $retorno;

    }
	
	public function retornarAvaVideo($idava){

        $this->sql = "SELECT `v`.`idvideo` AS `video_id`, `v`.`data_cad` AS `cadastrado_em`, `v`.`idvideoteca`, `a`.*, `m`.* FROM `avas_videotecas` AS `v`
        INNER JOIN `avas` AS `a` ON v.idava = a.idava
        INNER JOIN `videotecas` AS `m` ON m.idvideo = v.idvideoteca WHERE (v.ativo = 'S') AND (a.idava = '$idava')";
        $pastas = $this->retornarLinhasArray($this->sql);
        return $pastas;
    }
    ...
    public function cadastrarAnotacao($url) {
        if (verificaPermissaoAcesso(false)) {
            $this->sql = 'insert into
                            matriculas_anotacoes
                        set
            ...
            if ($this->executaSql($this->sql)) {
                ...
                $assunto = utf8_decode('Anotação enviada');
                $emailPara ='karina.feijo@ibrep.com.br';
				$emailPara2 = 'remota2@icetran.com.br';
				//$emailPara = 'andre.basilato@ibrep.com.br';
                $labelConteudo=utf8_decode('Conteúdo');
                $conteudos = $this->retornarConteudoEspecifico($this->post['idava'],$this->post['idobjeto']);
                $nome = utf8_decode($conteudos['nome']);

                $email =  "<strong>".utf8_decode("Dados do Conteúdo") . " </strong> <br>
				<strong>ID Do Ava: </strong>  " .$this->post['idava'] . " <br/>
				<strong>".utf8_decode("ID Da Matrícula:")." </strong>  " .$this->id . " <br/>
				<strong>".utf8_decode("Nome do Conteúdo:")." </strong>  $nome <br/>
				<strong>$labelConteudo: </strong>" . $this->post['anotacao'] . " <br>";

                $codAluno = [2, 3, 4, 27];

                if(in_array($this->post['idpessoa'], $codAluno)){
                    $this->enviarEmail($nomeDe, $emailDe, $assunto,$email, 'IBREP', $emailPara);
					$this->enviarEmail($nomeDe, $emailDe, $assunto,$email, 'IBREP', $emailPara2);
                }
