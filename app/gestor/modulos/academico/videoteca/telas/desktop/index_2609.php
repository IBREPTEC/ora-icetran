...
<?php
    ...
    foreach ($dadosArray as $dados) {
        ...
        $html .= '<div class="thumbnail" style="height: 40%;width: 100%">';
        if($dados['link']!='' || $dados['link'] != null) {
            $html .= '<iframe type="text/html" src="' . $dados['link'] . '"  allow="picture-in-picture" width="225" height="105" frameborder="0" allowfullscreen >';
        }
        if($dados['arquivo_servidor'] != ''){
            $html .= '<iframe type="text/html"  src="/storage/videoteca/' . $dados['arquivo_servidor'] . '"  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" width="225" height="105" frameborder="0" allowfullscreen >';
        ...
        if ('youtube' == $dados['variavel'] || 'vimeo' == $dados['variavel']) {
            ...
            $html .= '<img style="width: 50%;align:center" data-src="holder.js/160x120" alt="'.$dados['titulo'].'"  src="/storage/videoteca/'.$dados['video_imagem_servidor'].'">';
        }
        ...
        if ('html5' == $dados['variavel'] || 'interno' == $dados['variavel']) {
            if($config['videoteca_local']){
                $html .= '<video id="video-'.$dados['idvideo'] .'"
                        ...
                        poster="/storage/videoteca/'.$dados['idvideo'].'">';
                $html .= '<source src="/storage/videoteca/'.$dados['arquivo_servidor'].' " type="video/mp4" ></source>';
            } else {
                 $html .= '<video id="video-'.$dados['idvideo'] .'"
                        controls="controls" preload="none" height="345" width="420"
        ...
        if ('youtube' == $dados['variavel'] || 'vimeo' == $dados['variavel']) {
            $html .= '<video src="'.$dados['arquivo'].'?quality=540p" border="0" width="420" height="345" style="border: medium none"></iframe>';
        }
