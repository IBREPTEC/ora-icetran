...
$config['formulario_rotas_aprendizagem'] = array(
  array(
	...
	'campos' => array( // Campos do formulario
	  ...
	  array(
		"id" => "idava_rota_aprendizagem", // Id do atributo HTML
...
/*$config["listagem_rotas_aprendizagem"] = array(
  array(
	"id" => "idrota_aprendizagem",
	"variavel_lang" => "tabela_idrota_aprendizagem",
	"tipo" => "banco",
	"coluna_sql" => "idrota_aprendizagem",
	"valor" => "idrota_aprendizagem",
	"busca" => true,
  ...
  array(
	...
	"tipo" => "php",
	"valor" => 'return "<a class=\"btn dropdown-toggle btn-mini\" data-original-title=\"".$idioma["tabela_opcoes_tooltip"]."\" href=\"/".$this->url["0"]."/".$this->url["1"]."/".$this->url["2"]."/".$this->url["3"]."/".$this->url["4"]."/".$linha["idrota_aprendizagem"]."/opcoes\" data-placement=\"left\" rel=\"tooltip facebox\">".$idioma["tabela_opcoes"]."</a>"',
	"busca_botao" => true,
	"tamanho" => "80"
  )
 );*/
...
if($_POST["acao"] == "salvar_rota_aprendizagem"){
  ...
  /*if($_POST[$config["banco_rotas_aprendizagem"]["primaria"]])
	$salvar = $linhaObj->ModificarRotaAprendizagem();
  //else
	//$salvar = $linhaObj->CadastrarRotaAprendizagem();
...
} elseif($_POST["acao"] == "remover_rota_aprendizagem") {
  $linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|9");
  $linhaObj->Set("post",$_POST);
  $remover = $linhaObj->RemoverRotaAprendizagem();
  if($remover["sucesso"]){
...
if(isset($url[5])){
  /*if($url[5] == "cadastrar") {
	...
	if($linha) {
	  switch($url[6]) {
		...
		/*case "remover":
		  $linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|9");
		  include("idiomas/".$config["idioma_padrao"]."/remover.rotasdeaprendizagem.php");
		  include("telas/".$config["tela_padrao"]."/remover.rotasdeaprendizagem.php");
		break;*/
		case "opcoes":
		...
		case "objetos":
		  ...
		  //$_GET['q']['1|v.exibir_ava'] = 'S';
		  $videos = $linhaObjVideo->ListarTodasVideo();
		  //unset($_GET['q']['1|v.exibir_ava']);

		  /*
		  include("../classes/avas.simulados.class.php");
		  $linhaObjSimulado = new Simulados();
		  $linhaObjSimulado->Set("idava",intval($url[3]));
		  $linhaObjSimulado->Set("limite",-1);
		  $linhaObjSimulado->Set("ordem_campo","s.nome");
		  $linhaObjSimulado->Set("ordem","asc");
		  $linhaObjSimulado->Set("campos","s.*, a.nome as ava");
		  $_GET['q']['1|s.exibir_ava'] = 'S';
		  $simulados = $linhaObjSimulado->ListarTodosSimulados();
		  unset($_GET['q']['1|s.exibir_ava']);
		  */
		  ...
		  $linhaObj->Set("campos","arab.*,aa.nome as nome_audio, 
            ...
            ae.pergunta as nome_enquete, od.nome as nome_objeto_divisor, aex.nome as nome_exercicio");
  ...
  if(!$_GET["cmp"]) $_GET["cmp"] = $config["banco_rotas_aprendizagem"]["primaria"];
  $linhaObj->Set("ordem_campo",$_GET["cmp"]);
  $linhaObj->Set("campos","r.*, a.nome as ava");
  $dadosArray = $linhaObj->ListarTodasRotaAprendizagem();
  include("idiomas/".$config["idioma_padrao"]."/formulario.rotasdeaprendizagem.php");
  include("telas/".$config["tela_padrao"]."/formulario.rotasdeaprendizagem.php");*/
