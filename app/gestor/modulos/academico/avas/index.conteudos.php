...
elseif($_POST["acao"] == "salvar_imagem"){

    if($_FILES['arquivo']['size'] != '') {
        foreach ($_FILES as $ind => $val) {
            $_POST[$ind] = $val;
        }
        $validar = $coreObj->ValidarArquivo($_FILES["arquivo"]);
        ...
        if($validar || ($extensao != ".jpg" && $extensao != ".jpeg" && $extensao != ".gif" && $extensao != ".png" && $extensao != ".bmp" && $extensao != ".pdf" && $extensao != ".doc" && $extensao != ".docx")) {
            ...
            if($validar) {
                ...
                $retorno["mensagem"] = "mensagens_arquivo_extensao_erro";
            }
            ...
            $pasta = $_SERVER["DOCUMENT_ROOT"] . "/storage/avas_conteudos_imagem_exibicao";
            ...
            if ($envio) {
                ...
                $retorno = array('erro' => true, 'mensagem' => 'mensagens_arquivo_upload_erro');
                return $retorno;
            }
        }
    ...
    $linhaObj->Set("post",$_POST);
    $salvar = $linhaObj->cadastrarConteudoImagem($nomeServidor);
    if($salvar["sucesso"]){
...
}elseif($_POST['acao'] == 'editar_imagem'){
    $nomeServidor= "";
    ...
    $linhaObj->Set("post",$_POST);
    $salvar = $linhaObj->editarImagensConteudo($nomeServidor);
    if($salvar["sucesso"]){
...
if($url[8] == "remover_imagem"){
    echo $linhaObj->removeImagemConteudo($_GET['idimagem']);
    exit;
}
...
if(isset($url[5])){			
  if($url[5] == "cadastrar") {
	...
	if($linha) {
        if ($url[7] == "cadastrar_imagem") {


            include("idiomas/" . $config["idioma_padrao"] . "/opcoes.conteudos.php");
            include("telas/" . $config["tela_padrao"] . "/formulario.conteudos.imagens.php");
            exit;
        }elseif($url[7] == "editar_imagem"){
            $dados_imagem = $linhaObj->retornarDadosImagem($url[8]);
	  ...
	  switch($url[6]) {
		...
		break;
          case "imagens":
              $conteudos_imagens = $linhaObj->retornarImagensConteudo($url[3],$url[5]);

              include("idiomas/".$config["idioma_padrao"]."/imagens.conteudos.php");
              include("telas/".$config["tela_padrao"]."/imagens.conteudos.php");
          break;
		case "remover":			
		  $linhaObj->verificaPermissao($perfil["permissoes"], $url[2]."|9");
