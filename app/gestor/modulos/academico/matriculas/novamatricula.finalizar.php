...
if($_SESSION['matricula']['idcurso']==95){
    $config['formulario'] = $config['formulario_pessoas'];
    unset($config['formulario_pessoas'][2]['campos'][0]);
    unset($config['formulario_pessoas'][2]['campos'][1]);
    unset($config['formulario_pessoas'][1]['campos'][0]);
    $pessoaObj->Set('config', $config);
}
...
if ($pessoa['sucesso'] && $pessoa['id']) {
	...
	if ($matricula) {
		$pessoaObj->set('id', $pessoa["id"]);
    	$pessoaObj->set('campos', 'p.*');
    	$pessoaDados = $pessoaObj->retornar();

    	$celularPessoa = preg_replace('/[^A-Za-z0-9\-]/', "", $pessoaDados["celular"]);
    	$celularPessoa = str_replace("-", "", $celularPessoa);
    	$celularPessoa = substr_replace($celularPessoa, "55", 0, 0);
		
		$emailAutomaticoObj->sendWhatsApp($pessoaDados["nome"], $celularPessoa);
	}
