	...
	if($_GET["cpf"]){
		...
		if(!count($matricula["erros"])) {
			...
			if($_SESSION['matricula']['idcurso']==95){
				unset($matriculaObj->config['formulario_pessoas'][2]['campos'][0]);
				unset($matriculaObj->config['formulario_pessoas'][2]['campos'][1]);
				unset($matriculaObj->config['formulario_pessoas'][1]['campos'][0]);
