...
if($detranObj->obterSituacaoIntegracao((int)$GLOBALS['matricula']['escola']['idestado'])) {
    ...
    if (method_exists($detranObj, "DadosCertificado{$GLOBALS['matricula']['escola']['uf']}")) {
        ...
        if (count($dados) == 1)
			...
			$retornar_log = $detranObj->retornarLog($GLOBALS['matricula']['idmatricula']);
        if (!empty($retornar_log)) {
            $retorno_detran = strpos($retornar_log['retorno'], 'Verifique com Detran/SC');
        }
        $detran_certificado = $dados[0]['detran_certificado'];

      if($detran_certificado == 'N' && $retorno_detran === false){
            $matricula['detran']['exibir_botao_certificado'] = 'enviar';
        }elseif($detran_certificado == 'S' && $retorno_detran === false){
            $matricula['detran']['exibir_botao_certificado'] = 'reenviar';
        }
