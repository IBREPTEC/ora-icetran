...
exit();
    ...
    if($total_email == 1){
        ...
        if($usuario["idperfil"]){
			...
			$_SESSION["adm_idperfil"] = $usuario["idperfil"];
			
			if($usuario["gestor_cfc"] != "S") {
                $sql = "select e.idescola, i.idmantenedora from usuarios_adm_cfcs uac, escolas e, sindicatos i
                                   where uac.idusuario = '" . $usuario["idusuario"] . "' and uac.ativo = 'S'
                                   and uac.idcfc = e.idescola and e.ativo = 'S' and e.idsindicato = i.idsindicato";
                $queryCfcs = mysql_query($sql) or die (incluirLib("erro", $config, array("sql" => $sql, "session" => $_SESSION, "get" => $_GET,
                    "post" => $_POST, "mysql_error" => mysql_error())));
                $_SESSION["adm_cfcs"] = [];
                $_SESSION["adm_mantenedoras"] = [];

                while($cfc = mysql_fetch_assoc($queryCfcs)) {
                    $_SESSION["adm_cfcs"][$cfc["idescola"]] = $cfc["idescola"];
                    $_SESSION["adm_mantenedoras"][$cfc["idmantenedora"]] = $cfc["idmantenedora"];
                }

                $_SESSION["adm_cfcs"] = implode(",", $_SESSION["adm_cfcs"]);
                $_SESSION["adm_mantenedoras"] = implode(",", $_SESSION["adm_mantenedoras"]);

                $sqlEscolas = "select idescola from escolas where ativo = 'S' and idescola in (" . $_SESSION["adm_cfcs"] . ") ";
                $resultadoEscolas = mysql_query($sqlEscolas);
                while ($escola = mysql_fetch_assoc($resultadoEscolas)) {
