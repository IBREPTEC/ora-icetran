...
<script type="text/javascript">
    ...
    if( !$cargaCompleta && ( $objeto['idobjeto'] == $ultimoConteudoRota ) && $ava['contabilizar_datas'] == 'S' ){
        if( $ava['carga_horaria_min'] > 0 ){
            echo "alert("."'".sprintf($idioma['carga_incompleta'],$ava['carga_horaria_min'])."'); window.history.back();";
        }
    }
    ...
    if($objeto['objeto_proximo']['idobjeto']) {
        ...
        if(!$preRequisito) echo "preRequisito();";
            ...
            echo "if (Date.now() > tempoMinimo) {window.location = \"/{$url[0]}/{$url[1]}/{$url[2]}/{$matricula['idmatricula']}/{$ava['idava']}/rota/{$anterior1}\";} else {alert('{$idioma['tempo_minimo']}');}";
        } else {
    ...
    {
        e.preventDefault();
        var idObjeto = <?php echo $objeto['objeto_proximo']['idobjeto']; ?>;
        ...
        if(ultimoConteudo === idObjeto && simulados > 0 && simuladosRealizados === 0) {
            alert('É necessário realizar o simulado deste módulo para continuar!');
            redireciona = false;
        }
    }
    ...
    <?php if($_GET['alert'] == 'verificarSimulados'){
        echo "alert('É necessário realizar o simulado deste módulo para continuar!');";
