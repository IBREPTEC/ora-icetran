    ...
    <div class="row container-fixed">
        ...
        <div class="box-side box-bg half-side-box">
            <div class="top-box box-verde">
                <h1><?php echo $idioma['conteudo']; ?></h1>
                <?php if($favorito) { ?>
                ...
                <?php if($contabilizado) { ?>
                    <i id="icone_contabilizado" class="icon-ok-sign"></i>
                <?php } else { ?>
                    ...
                    <div class="abox m-box">
                        <div class="nav-breadcrumb">
                            <div class="nav-breadcrumb-item first-nav">
                                ...
                                <?php } else { ?>
                                    ...
                                    $maior = count($idAvas);                                 
                                     $anterior = $ava['idava'];
                                   for ($a = $maior -1 ; $a >= 0 ; $a--) {                                         
                                        if($anterior > $idAvas[$a]['idava']){
                                            $anterior = $idAvas[$a]['idava'];
                                            break;
                                        }                                       
                                   }                                   
                                ?>
                            ...
                            <div class="nav-breadcrumb-item last-nav">
                                ...
                                <?php } else { ?>
                                <?php
                                 // echo "valor Ava " . $ava['idava'] . '<br />';
                                
                                   $proximo = $ava['idava'];
                                   foreach ($idAvas as $key => $value) {                                   
                                    if(  $proximo < $idAvas[$key]['idava']){
                                          $proximo = $idAvas[$key]['idava'] ;
                                          break;
                                    }
                                } ?>
                                <?php if($proximo == $ava['idava'] )   {?>
                                   ...
                                   <?php }else{ ?> 
                                         <a  href="<?php echo '/'.$url[0].'/'.$url[1].'/'.$url[2].'/'.$matricula['idmatricula'].'/'. $proximo.'/rota/1#conteudo';?>">
                                        <hr>
                                        <div><i class="icon-chevron-right"></i></div>
                                        <p><?php echo $idioma['proximo']; ?></p>
                                    </a>
							...
							switch ($objeto['tipo']) {
                                ...
                                case 'video':
                                    ...
                                    if ('html5' == $objeto['objeto']['variavel'] || 'interno' == $objeto['objeto']['variavel']) {
                                        ...
                                        $html .= '<iframe src="'.$objeto['objeto']['arquivo'].'" webkitallowfullscreen mozallowfullscreen allowfullscreen border="0" width="100%" height="400px" style="border: medium none;"></iframe>';
                                    }
...
<script type="text/javascript">
	...
	<?php if(!$contabilizado) {
		...
		function contabilizar() {
			$.post('/<?php echo $url[0]; ?>/<?php echo $url[1]; ?>/<?php echo $url[2]; ?>/<?php echo $url[3]; ?>/<?php echo $url[4]; ?>/json/contabilizar',
				{
     ...
     function verificarDias(e)
    {
        ...
        <?php if(! $preRequisitoDias['sucesso']){ ?>
            alert('Opa! Você atingiu a visualização máxima de 10h permitida por dia. Você pode continuar a acompanhar o conteúdo amanhã, dia ' + '<?= $preRequisitoDias['data'] ?>!');
            redireciona = false;
        <?php } ?>
    }
