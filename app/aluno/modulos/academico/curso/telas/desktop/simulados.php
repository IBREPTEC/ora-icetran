                            ...
                            <div class="span12 center-align">
                                <table width="100%" border="0" cellspacing="1" cellpadding="5">
                                    ...
                                    <tbody class="b-table">
										...
										<?php $avaliacao = "$url[0]/$url[1]/$url[2]/$url[3]/$url[4]/avaliacoes"; } ?>
                                    </tbody> 
                                </table><br />
                                <div class="ui raised segment">
                                    <hr />
                                    <h2 style="text-align: center;">Avaliação do módulo</h2>
                                    <hr />
                                    <?php
                                    ...
                                    foreach ($retornaConteudo as $conteudo) {
                                        ...
                                        foreach ($rotasDeAprendizagens as $rotaDeAprendizagem) {

                                                $contabilizado =$matriculaObj->totalContabilizado($conteudo['objeto']['idava'], 'objeto', $rotaDeAprendizagem['idobjeto']);

                                                if($rotaDeAprendizagem['tipo'] != 'objeto_divisor'){
                                                    $pagina ++;
                                                }

                                        } ?>
                                        <h4><p style="text-align: center;">Já realizou o simulado e está preparado para realizar sua prova? BOA SORTE!</p></h4>

                                        <?php if($conteudo['objeto'] != 'Falta Reconhecimento Facil'){?>
                                            <a href="/<?= $url[0] ?>/academico/curso/<?= $matricula["idmatricula"]; ?>/<?=$conteudo['objeto']['idava'];?>/rota/<?=$pagina?>#conteudo">
                                                <div class="btn btn-amarelo btn-mob" style="margin-bottom: 3rem;">Clique Aqui</div>
                                            </a>

                                        <?php }else{?>
                                            <?php echo  "<script>alert('Falta reconhecimento facial');
                                            window.location.href =\"/{$url[0]}/academico/curso/{$matricula['idmatricula']}/{$conteudo['objeto']['idava']}/?opBiometria=sim\";
