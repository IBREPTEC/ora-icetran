...
if (!empty($url[3])) {
    ...
    if (isset($url[4]) && $url[5] <> 'avaliacoes') {
            ...
            $retornoAcesso = $gestaoAcessosObj->contabilizarAcessoMatricula($matricula["idpessoa"], $idmatricula, $url[4], $cursoNome["idcurso"]);
        }
    } else {
            ...
            if (!$aceito) {
                ...
                echo '<script>
					if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
                        $("#contrato_aceito").removeAttr("hidden")
               		}else{
