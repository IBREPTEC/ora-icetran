...
if ((int) $url[6]) {
	...
	$imagens = $conteudo->retornarImagensConteudo($ava['idava'],$objeto['idconteudo']);
	...
	$ava_traducao["modulos"] = unserialize($ava["modulos"]);
	$traducao = in_array('traducao', $ava_traducao['modulos']);

    if ($objeto['idobjeto']) {
        $anterior = $ava['idava'] == 117 ? (int)$url[6] - 2 : (int)$url[6] - 1;
        $proximo = (int)$url[6] + 1;
        ...
        $informacoesTopoCurso['pagina'] = $objeto['objeto']['nome'] ;
        $informacoesTopoCurso['modulo'] =  $ava['disciplina'];
        $informacoesTopoCurso['matricula'] = $matricula;
        $informacoesTopoCurso['ava'] = $ava['idava'];
        ...
        switch ($objeto['tipo']) {
            case 'conteudo':
				...
				preg_match_all('/\[\[video\]\[(\d+)\]\]/i', $objeto['objeto']['conteudo'], $matches);
				$information = [];

				foreach ($matches[0] as $position => $tag) {

					$information = $videoTeca->retornarAvaVideo($matches[1][$position]);

				}

				$tipoLink = $objeto['objeto']['tipo_link'];
				$frame = '';
				switch ($tipoLink){
					case 'J':
						$frame = '<iframe src="' . $information['link_2'] . '?quality=540p" webkitallowfullscreen mozallowfullscreen allowfullscreen border="0"
		width="100%" height="400" style="border: medium none"></iframe>';

				   break;
					case 'V':
						$frame = '<iframe src="' . $information['link_1'] . '?quality=540p" webkitallowfullscreen mozallowfullscreen allowfullscreen border="0"
		width="100%" height="400" style="border: medium none"></iframe>';
					break;
