	...
	case "fazer":
		if (verificaPermissaoAcesso(true)) {
			...
			if(
				...
				$validadoReconhecimento = explode('.', $imagemPrincipal['foto']);
				$recon = md5($validadoReconhecimento[0] . date('d/m/Y H'));
				if (isset($_GET['reconhecimento']) && $recon == $_GET['reconhecimento'] ||
                    $curso['usar_datavalid'] == 'N' || $dadosSindicato['usar_datavalid'] == 'N' ||
                    $matricula['matricula_liberada_azure'] === 'S') {
